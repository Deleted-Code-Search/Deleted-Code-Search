...
2. **혼재 쌍의 건수와 비율을 보고한다.** 블록별로, 그리고 500건 전체 합계로. 혼재 비율이 높은 블록은 **"버전별 비교 불가"로 표시**하고 그 블록의 버전별 kappa를 결론에 쓰지 않는다.
...
> **v2 기준 상태.** 2(신뢰도 구간)는 ADR-012로 확정됐고, 16(예비 200건 포함 여부)은 v2에서 결정했다. 13(테스트 특례)은 §5.1 규칙 ②·③으로 보강했다. 17은 v2에서 새로 생긴 항목이다. **1(우선순위표)과 8(소요 시간)은 여전히 미확정이고 500건 시작 전에 필요하다.** 나머지는 그대로다.
