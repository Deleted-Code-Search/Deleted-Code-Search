...
`guide_version: v2` · 갱신: 2026-09-24 · 담당: 성제 (sj) · 근거: CHARTER.md §4.2 ③, §4.4, §10.2, §11, ADR-004, ADR-005, ADR-012, ADR-014, ADR-017, ADR-018
...
6. [근거 등급 판정 규칙](#6-근거-등급-판정-규칙) — **§6.1.1 EXPLICIT 세 조건 / §6.2.1 INFERRED 근거 6종 / §6.3.2 원인 태그 (전부 v2 신설)**, §6.3.3 `filter-miss` 기준 (#132)
...
- **포함되지 않음**: 기능 자체가 없어진 것(FEAT — 사용자가 할 수 있던 일이 사라졌나로 구분). 순수 이동·리네임(필터가 `NOISE_MOVE`·`NOISE_RENAME`로 걸러야 한다. 라벨링 단계에서 만났으면 `note`에 `filter-miss` 태그를 남기고 재헌에게 알린다. **본문이 거의 그대로일 때만이다** — 같은 자리에서 다시 쓴 것은 해당하지 않는다, §6.3.3). `refactor` 단어만 있고 더 구체적인 이유가 적혀 있는 것(그 이유를 따른다, §3.2).
...
1. **두 신뢰도를 섞지 않는다.** `replacement.confidence`는 파이프라인이 "이 코드가 삭제된 함수의 대체가 맞다"에 갖는 확신이다 (`pipeline/context.py`: 삭제 자리(±2줄)에서 하나로 확인되면 0.9, 아니면 0.7 (#111)). 라벨의 `confidence`는 라벨러가 매기는 INFERRED 근거의 강도다 (ADR-012 구간). **앞의 값을 뒤에 옮겨 적지 않는다.** 대체가 맞다는 것과 그 대체가 이유를 드러낸다는 것은 다른 주장이다.
...
**예비 200건 파일은 `replacement`가 200건 모두 `null`이었다** (게이트 1은 이 상태로 측정했다). 대체 코드 매칭이 그 뒤에 붙었다 (#67 → PR #103, #107 → PR #108). 500건 파일은 `replacement`가 채워진 채로 나온다. **다만 채워진다는 것이 `code`가 있다는 뜻은 아니다.** 같은 예비 200건을 새 파이프라인으로 다시 돌린 결과가 이렇다 (**스냅샷: #111 시점, 예비 200건 기준.** 입력은 PR #103·#108 재실행과 같고, `confidence` 열은 #111(PR #126)이 머지 전 다시 돌려 확인한 값이다):

| `match_method` | 건수 | `replacement.confidence` | `code` | 뜻 |
|---|---:|---|---|---|
| `SAME_LOCATION` | 39 | 0.9 28건 / 0.7 11건 | 39건 채워짐 (자식 파일 원문을 넘겼을 때. 안 넘기면 8건) | 삭제된 함수와 같은 이름의 함수가 같은 커밋·같은 파일에 추가됐다 |
| `NONE` | 81 | 0.0 | 항상 `null` | 그 커밋이 그 파일에 추가한 줄이 **하나도 없다** |
| `null` | 80 | 0.0 | 항상 `null` | **판정 불가.** 이름이 다른 함수가 들어왔거나, 부분 수정뿐이거나, 판정 재료가 없다 |

#111(PR #126)로 반영됨. 삭제 자리 밖의 단일 후보 8건이 0.9 → 0.7로 내려가 #120 시점의 0.9 36 / 0.7 3이 28 / 11이 됐다. `code`·`match_method`는 그대로다.
...
3. **순수 이동·리네임으로 보이는 경우(filter-miss).** 삭제가 아닌 것이 필터를 통과한 것이므로 라벨 문제가 아니다 → `UNK` + `note`에 **`filter-miss`**. 게이트 1에서 18건(9%) 나왔고 필터 정밀도 재측정(CHARTER §10.1)의 입력이다. **단, 그 18건 중 9건은 이동이 아니었다** (#80). 무엇이 순수 이동·리네임인지는 **§6.3.3**이 정한다 — 같은 자리에서 크게 다시 쓴 것, 옮기면서 크게 다시 쓴 것은 해당하지 않는다.
...
**최소 1개 필수.** UNKNOWN 라벨은 4종 중 최소 1개를 반드시 단다. **예외는 `filter-miss` 하나다** — 이 건은 애초에 분석 대상이 아니라고 본 것이라 "무엇이 없어서 못 했나"를 물을 대상이 아니다. `filter-miss`가 달린 UNKNOWN은 원인 태그를 강제하지 않는다. **보고서는 이 건을 원인 분포에서 뺀다** (#134) — `classify/labels.py`의 `unknown_causes()`는 원인 태그 없이 `filter-miss`만 단 UNKNOWN을 `(태그 없음)`이 아니라 `filter-miss` 칸으로 따로 세고, 보고서의 "UNKNOWN 원인"은 원인 분포(4종 + `(태그 없음)`) 아래에 `filter-miss (원인 태그 없이): N건 — 원인 분포에서 제외(§6.3.2)`를 따로 찍는다. 그래서 `(태그 없음)`에는 진짜 태그 누락만 남는다. `filter-miss` 판정은 `label_cli` 저장 검증과 같은 낱말 매칭이다(§6.3.3). 원인 태그와 함께 단 `filter-miss`는 원인 태그로만 센다. **#88은 "4종 중 1개 이상 **또는** `filter-miss`"를 조건으로 구현한다.**

`filter-miss`는 원인 태그 4종에 **들어가지 않는다.** 필터 정밀도(§10.1)를 재는 관찰 태그이고, 위 예외 조항에서만 원인 태그의 자리를 대신한다. 원인 태그와 함께 달아도 된다.

**UNKNOWN을 부끄러워하지 말라.** §11의 최대 리스크는 **이유 회수율 저조**이고, 그것은 UNKNOWN을 다른 라벨로 채우면 **측정 자체가 무의미해진다.** 게이트 1은 "우리가 얼마나 잘 라벨했나"를 재는 게 아니라 "이 데이터에 이유가 있나"를 재는 지점이다. 회수율 45%는 나쁜 소식이지만 대응할 수 있는 소식이고(저장소 기준 강화 또는 축 이동), UNKNOWN을 DESIGN으로 채워 만든 회수율 85%는 6주차 게이트 2에서 분류 정확도로 되돌아와 프로젝트를 잘못된 방향으로 끌고 간다.

#### 6.3.3 `filter-miss`는 순수 이동·리네임에만 단다 (#132)

**`filter-miss`는 "배울 게 없는 삭제인데 필터를 통과한 것"을 사람이 보고 표시하는 태그다.** 달 수 있는 경우는 **순수 이동·리네임** 하나다 — 지워진 본문이 **거의 그대로** 다른 곳에 있다. 기록 방식은 그대로다: `UNK` + `UNKNOWN` + `note`에 `filter-miss` (§6.3.1-3, §6.3.2 예외 조항).

**왜 다시 적나.** 예비 200건에서 `filter-miss`가 붙은 18건을 #80에서 재헌이 전수 조사했더니 **9건은 이동이 아니었다** (대부분 같은 자리 재작성). 라벨러마다 쓰는 법도 달랐다 — 태그가 붙은 19줄 중 hs 12줄은 모두 `UNK` + `filter-miss`, sj 7줄은 전부 `UNK`가 아닌 이유 라벨에 `filter-miss 의심`·`이동으로 볼 때 filter-miss`를 덧붙였고, jh는 0줄이었다. sj의 7줄 중에는 "시그니처만 바뀌어 남은 수정일 가능성"처럼 **제자리 수정을 의심한 건**도 있었다. 이대로면 `filter-miss` 건수는 필터가 놓친 수가 아니라 라벨러의 습관을 센다.

| 경우 | `filter-miss` | 어떻게 라벨하나 |
|---|---|---|
| 본문이 거의 그대로 **다른 파일·위치**에 있다 (순수 이동) | **단다** | `UNK` + `UNKNOWN` + `filter-miss`. 옮겨 간 곳을 `note`에 적는다 |
| 본문이 거의 그대로이고 **이름만** 바뀌었다 (리네임, 이동+리네임) | **단다** | 위와 같다 |
| ADR-014 유사도 0.9에 **조금 못 미쳐도** 사람이 보기에 이동·리네임이다 | **단다** | 위와 같다. 유사도를 계산하지 않는다 — 눈으로 본다 |
| 같은 커밋이 아니라 **가까운 다른 커밋**에서 그대로 다시 나타났다 | **단다** | 위와 같고, **어느 커밋에서 다시 나타났는지**(SHA) `note`에 적는다. GitHub에서 찾았으면 §2.3대로 `off-record-evidence`도 남긴다 |
| **같은 자리에서** 크게 다시 썼다 | 달지 않는다 | 대체 코드가 있는 정상 삭제다. 이유를 평소대로 라벨한다 (§3) |
| **옮기면서** 크게 다시 썼다 | 달지 않는다 | 코드가 바뀌었으므로 배울 게 있다. 평소대로 |
| 대응 구현이 **부모 커밋에 이미** 있었다 | 달지 않는다 | 이 커밋의 이동이 아니다. 평소대로 |
| 실제 삭제 — 대응 코드가 어디에도 없다 | 달지 않는다 | 평소대로 |

**경계 예시 — "거의 그대로"와 "크게 다시 씀".** 전부 #80 재헌 조사에서 가져왔고, 판정 문구는 그 코멘트 그대로다 (여기서 새로 판정하지 않았다). 모두 `pydantic/pydantic`이고 `record_id`는 앞 8자리다. **이 예시들의 이유 라벨은 여기서 정하지 않는다** — `filter-miss`를 다느냐만 보여 준다.

*달았어야 하는 것 — #80 판정 A, 현행 `NOISE_MOVE` 기준의 실제 miss*

| `record_id` | 함수 | 커밋 | #80 판정 |
|---|---|---|---|
| `4aafd867` | `validate` | `75609f19` | "`validate` → `_validate` 이동+리네임. 대응 코드는 확인되지만 유사도 **0.8889**로 현행 임계값 0.9 바로 아래" |
| `dd5e735a` | `not_none_validator` | `35953d20` | "삭제와 재등장이 동일 커밋이 아니라 **2커밋 떨어져 있음**." 이 경우가 `note`에 재등장 커밋을 적어야 하는 경우다 |

참고로 같은 A의 `283e72d9`(`test_forward_ref_sub_types`, 유사도 0.9524)는 ADR-014 **안**에서 필터가 놓친 것이라 #80에서 필터를 고쳤다. 500건에서는 필터가 거른다.

*달지 않는 것 ① — 옮기면서 크게 다시 씀 (#80 판정 A 중 "현행 ADR-014 정의 밖" 5건에서)*

| `record_id` | 함수 | 커밋 | #80 판정 |
|---|---|---|---|
| `c57aba46` | `test_schema_as_string` | `de3175c1` | "다른 위치에 대응 코드가 있으나 본문 재작성 폭이 커 현행 ADR-014 유사도 기준(0.9) 미달" |
| `11c2ad0f` | `generate_computed_field` | `45259115` | "목적지는 확인되지만 이동 과정에서 본문 변경이 커 현행 ADR-014 유사도 기준 밖" |
| `9d5afbb3` | `__getattribute__` | `57f9d5ff` | "대응 목적지는 있으나 본문 재작성 폭이 커 현행 ADR-014 유사도 기준 밖" |

`4aafd867`과 이 셋은 모두 0.9 미만이다. **가르는 것은 수치가 아니라 본문을 다시 썼느냐다.** `4aafd867`은 이름이 바뀐 것 말고는 본문이 거의 그대로이고, 이 셋은 목적지가 있어도 본문이 다시 쓰였다. #80 코멘트에는 이 5건의 유사도 수치가 없으므로 "0.8 아래면 재작성" 같은 숫자 기준을 세우지 않는다.

*달지 않는 것 ② — 이동이 아님 (#80 판정 B 9건에서)*

| `record_id` | 함수 | 커밋 | #80 판정 | 해당하는 줄 |
|---|---|---|---|---|
| `04115395` | `benchmark_recursive_model` | `853f3702` | "동일 위치에서 함수가 크게 재작성된 사례. 이동 목적지 없음" | 같은 자리 재작성 |
| `1c56c4c2` | `checker` | `825a6920` | "등가/관련 구현이 이미 parent commit에 존재. 해당 commit에서 새 목적지로 이동한 사례가 아님" | 부모 커밋에 이미 있음 |
| `cbfc7dfb` | `convert_generics` | `594effa2` | "실제 삭제로 확인. 대응되는 이동 목적지 없음" | 실제 삭제 |

B 9건 중 같은 자리 재작성이 가장 많다(`04115395` 외에 `d65b12c9`·`ad389fd1`·`2d279b05`·`1b0b6cb9`·`cb4ec095`). 재헌은 `04115395`를 "짧은 함수의 교체가 FULL_FUNCTION 삭제로 추출되어 `filter-miss`로 보인 것"으로 봤다. **제자리면 이동이 아니다** — ADR-014 결정 3도 같은 경로·같은 위치를 이동 후보에서 뺀다.

**판단이 서지 않으면 달지 않는다.** 평소대로 라벨하고(UNK면 §6.3.2 원인 태그도 평소대로), `note`에 사유를 적는다. #80 판정 C인 `717c77ac`(`any_schema`, "관련 구현의 교체/재작성 성격이 강해 명확한 이동/리네임으로 판정하기 어려움")가 이런 경우다. **이때 `note`에 `filter-miss` 낱말을 쓰지 않는다.** `label_cli`는 저장할 때 `note`에서 태그를 낱말로 찾으므로 `filter-miss 의심`도 `filter-miss`로 보고, 원인 태그 없는 UNKNOWN을 그대로 통과시킨다. `이동 의심: pydantic/_internal/x.py의 y` 처럼 적는다.

**이 절은 이동·리네임만 다룬다.** 포맷만 바뀐 것, 생성 코드 등 다른 노이즈 유형이 필터를 통과한 것 같으면 `filter-miss`를 달지 않고 `needs-discussion`에 사유를 적는다.

**왜 이 경계인가.** 이 프로젝트가 찾는 것은 "왜 지웠고 대신 무엇이 들어갔나"다 (CHARTER §2.2). 같은 자리에서 크게 다시 쓴 삭제는 그 전형이다 — 지워진 코드와 대체 코드가 한 자리에 나란히 있다. 이것을 `filter-miss`로 빼면 배울 게 가장 많은 건을 버리고 필터 정밀도도 실제보다 낮게 잰다. 게이트 1 예비 200건에서 `filter-miss` 18건 중 9건이 바로 이 경계를 넘었다 (#80). **이것은 ADR-014를 바꾸는 것이 아니다.** ADR-014는 필터가 **자동으로** 거르는 기준이고, `filter-miss`는 사람이 필터의 **놓침**을 표시하는 기준이다. 그래서 두 기준은 다를 수 있고, 달라야 쓸모가 있다. `4aafd867`처럼 0.9 바로 아래의 이동+리네임과 `dd5e735a`처럼 커밋을 건너간 이동을 `filter-miss`로 표시해 두면, #80에서 보류한 "ADR-014를 넓힐지"를 #89 결과로 판단할 때 그 근거가 된다.

**#89 필터 정밀도 판정도 이 경계를 쓴다.** 이 경계는 CHARTER §15 "의미 있는 삭제"(이동·리네임 등을 제외한 배울 게 있는 삭제)와 같은 말이다 — 같은 자리 재작성·옮기며 다시 쓴 것·실제 삭제는 의미 있는 삭제이고, 순수 이동·리네임은 아니다. CHARTER §10.1의 정밀도("통과 중 진짜 의미 있음 비율")를 판정할 때 이동·리네임 여부는 이 절대로 가른다. 판정 절차 자체는 #89에서 설계한다 (`docs/evaluation.md`).
...
| v2 (유지) | 2026-09-24 | **#111(PR #126) 대체 코드 신뢰도 규칙·분포 반영 (#123).** ① §6.2.2 근거 ① 구간 상한 1번의 `replacement.confidence` 설명을 "삭제 자리(±2줄)에서 하나로 확인되면 0.9, 아니면 0.7"로 고침(전: 같은 이름 후보가 하나면 0.9) ② §6.2.3 분포 표 `SAME_LOCATION` 0.9 / 0.7을 36 / 3 → 28 / 11로, 스냅샷을 "#111 시점, 예비 200건 기준"으로 ③ 선행 조건 표 7번 0.9 건수 36 → 28. `NONE` 81·판정 불가 80은 그대로다. **ADR-017·ADR-012·이유 8종·라벨 스키마는 그대로다** | 없음 (v2 라벨 0건, 예비 200건은 v1) | 성제 |
| v2 (유지) | 2026-09-24 | **`filter-miss` 기준을 순수 이동·리네임으로 좁힘 (#132).** #80 전수 조사에서 예비 200건 `filter-miss` 18건 중 9건이 이동이 아니었고(대부분 같은 자리 재작성) 라벨러마다 쓰는 법이 달랐다. ① §6.3.3 신설 — 다는 경우(본문이 거의 그대로인 이동·리네임, 0.9에 조금 못 미치는 것, 가까운 다른 커밋으로의 이동 — 재등장 커밋을 `note`에)와 달지 않는 경우(같은 자리 재작성, 옮기며 재작성, 부모 커밋에 이미 있음, 실제 삭제), #80 재헌 판정을 인용한 경계 예시(A 2건 + 0.9 미만 재작성 3건 + B 3건 + C 1건), 판단이 안 서면 달지 않고 `note`에 `filter-miss` 낱말도 쓰지 않는다, 이 경계를 택한 이유, #89 정밀도 판정도 이 경계를 쓴다 ② §4 DESIGN·§6.3.1-3에서 §6.3.3을 가리킴 ③ §6.3.2의 "원인 분포 집계에서도 뺀다"를 실제 동작에 맞춤 — 보고서(`unknown_causes()`)는 `filter-miss`만 단 UNKNOWN을 `(태그 없음)`으로 센다. 유효한 라벨임을 명시하고 보고서 수정은 #134로 분리. **태그 어휘·기록 방식(`UNK` + `UNKNOWN` + `filter-miss`)·`label_cli` 검증·ADR-014·이유 8종·라벨 스키마는 그대로다.** 판정 경계를 좁혔지만 v2로 저장된 라벨이 0건이라 `guide_version`은 v2를 유지한다 | 없음 (v2 라벨 0건, 예비 200건은 v1) | 성제 |
| v2 (유지) | 2026-09-24 | **UNKNOWN 원인 보고서에서 `filter-miss`를 원인 분포와 분리 (#134).** ① `classify/labels.py` `unknown_causes()`가 원인 태그 없이 `filter-miss`만 단 UNKNOWN을 `(태그 없음)`이 아니라 `filter-miss` 칸으로 센다. 판정은 `label_cli` 저장 검증(#88)과 같은 낱말 매칭(`has_note_tag`)이다. 원인 태그와 함께 단 `filter-miss`는 원인 태그로만 센다 ② 보고서 "UNKNOWN 원인"이 원인 분포(4종 + `(태그 없음)`) 아래에 `filter-miss (원인 태그 없이): N건 — 원인 분포에서 제외(§6.3.2)`를 따로 찍는다. 예비 200건 기준 `(태그 없음)` 43 → 31, `filter-miss` 12건 분리, 원인 태그 4종 건수는 그대로 ③ §6.3.2의 "`(태그 없음)`으로 나온다, 수정은 #134" 문장을 새 동작으로 고침. **태그 어휘·기록 방식·`label_cli` 검증 규칙·이유 8종·라벨 스키마는 그대로다** | 없음 (보고서 집계만 바뀜, 라벨 값은 그대로) | 성제 |
...
| 7 | **(가능하면) `replacement.confidence`를 라벨러 화이트리스트에 넣는다** | §6.2.2 근거 ① 구간 상한이 이 값에 기댄다. 보이지 않으면 4번 규칙(0.8 미만 간주)이 **모든 근거 ①을 0.5~0.8로 묶는다.** 파이프라인은 `SAME_LOCATION` 39건 중 28건에 0.9를 준다(#111 시점, 예비 200건 기준). 넣을 때는 §2.1 표를 함께 고친다 (§2.1 "셋이 함께 바뀌어야 한다") | 희수 (`classify/sampling.py`) — **완료 (#117 → PR #120).** 화이트리스트와 label_cli 대체 코드 머리 줄에 함께 들어갔다. §2.1 표·§6.2.2 4번은 #123에서 반영. 4번은 이제 `confidence`가 없는 옛 파일용 대비책이다 |

**이 문서는 라벨러 3인이 모두 읽어야 한다.** 특히 §5.1(규칙 3개), §6.1.1(EXPLICIT 세 조건), §6.2.1(INFERRED 근거 6종), §6.3.2(원인 태그)는 v1에 없던 것이라 읽지 않으면 v1 기준으로 라벨하게 된다. **2026-09-24 반영분(#115)도 읽는다** — §2.1(보이는 필드), §6.0(`pr_labels`), §6.2.2 근거 ① 구간 상한, §6.2.3(`replacement` 읽는 법), §7.2.1(리뷰 locator). **§6.3.3(`filter-miss` 기준, #132)도 읽는다** — 예비 200건에서 라벨러마다 뜻이 달랐던 태그다.
