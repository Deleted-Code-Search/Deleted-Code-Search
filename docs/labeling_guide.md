...
`guide_version: v2` · 갱신: 2026-09-23 · 담당: 성제 (sj) · 근거: CHARTER.md §4.2 ③, §4.4, §10.2, §11, ADR-004, ADR-005, ADR-012
...
> 그래서 v2가 바꾼 것은 **근거 등급의 적용 기준**이다: EXPLICIT의 도달 검사(§6.1), INFERRED로 인정되는 근거의 종류(§6.2), UNKNOWN의 적극적 정의와 원인 태그(§6.3). 세 등급의 **정의 자체는 CHARTER §4.2 ③ 그대로**이고 이 문서가 바꾸지 않는다 (§10.4).
...
**INFERRED의 근거는 아래 6종뿐이다.** 이 중 하나도 못 대면 UNKNOWN이다.
...
4. **그 50건을 새 버전으로 재라벨한다.** 나머지 450건은 새 규칙으로 계속 진행한다.
...
**재라벨 kappa의 한계를 적어 둔다.** 같은 사람이 같은 50건을 다시 보므로 **기억 때문에 완전히 독립이 아니다.** 그래서 재라벨 kappa는 "새 규칙이 작동하는가"의 참고치로만 쓰고, **§9 5주차의 최종 kappa 보고는 재라벨하지 않은 나머지 450건으로 낸다.** 재라벨한 50건의 수치는 따로 표시해 함께 싣는다.
...
### 10.4 v2가 CHARTER를 건드리는가 — 판단

**건드리지 않는다. CHARTER.md는 이 PR에서 수정하지 않았다.**

CHARTER §4.2 ③의 근거 등급 정의는 이렇다.
...
| v2가 정한 것 | 정의를 바꾸나 |
|---|---|
| §6.1.1 — "이유"가 **이 레코드의 삭제 이유**를 뜻하고, "있음"은 도달 검사로 판정한다 | **아니다.** 정의가 말하지 않은 판정 방법을 정한 것이다. 우리 데이터 단위가 `DeletionRecord` 1건이므로(§4.4, §2) "이유"가 그 레코드의 삭제 이유라는 것은 정의의 해석이지 변경이 아니다 |
| §6.2.1 — INFERRED 근거 6종 | **아니다.** CHARTER가 이미 "대체 코드·테스트·호출 변화"를 열거했다. 6종 중 ①②③이 그것이고, ④⑤⑥은 같은 성격(diff에서 확인 가능한 구조적 신호)의 세분이다 |
| §6.3.1 — UNKNOWN 네 경우와 원인 태그 필수 | **아니다.** "판단 불가"의 범위를 좁히거나 넓히지 않았다. 언제 그렇게 판정하는지와 무엇을 함께 기록하는지를 정했다 |
| 등급 3종의 이름·개수·순서 | **그대로다** |
| ADR-012의 0.5 / 0.8 경계 | **그대로다** (§6.2.2) |
| 이유 8종 | **그대로다** |

→ **적용 기준 명확화다. §13 절차(이슈 → 회의 → ADR → CHARTER 갱신) 대상이 아니다.**

**다만 실질적 효과는 솔직하게 적는다.** 적용 기준을 좁혔으므로 **같은 데이터에서 EXPLICIT으로 인정되는 범위가 v1보다 좁아진다.** 정의는 그대로인데 결과 숫자는 달라진다. 그래서:
- v1 수치와 v2 수치를 **"EXPLICIT만" 구간에서 직접 비교하면 안 된다** (§6.1.3).
- 게이트 1 판정은 EXPLICIT+INFERRED(≥ 0.5) 구간으로 했고 이 구간은 E↔I 재판정에 영향받지 않으므로(`docs/meetings/gate-1.md` 판정 근거 3), **게이트 1 판정은 그대로 유효하다.**
- 이 판단에 이견이 있으면 PR 리뷰에서 받는다. **"정의를 바꾼 것"이라는 판단이 우세하면 §13 절차로 돌린다** — 그 경우 이 문서는 v2로 내보내지 않고 보류한다.
