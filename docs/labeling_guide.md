...
> **이 표와 화이트리스트는 함께 바뀌어야 한다.** 한쪽만 고치면 라벨러가 보는 것과 가이드가 가정하는 것이 어긋난다. #99에서 `pr_number`·`issue_numbers`가 화이트리스트에 빠져 있어, 라벨러가 커밋 메시지 끝의 `(#10623)`을 눈으로 주워 `evidence_locator`를 적은 전례가 있다. 표를 고치는 PR은 화이트리스트를, 화이트리스트를 고치는 PR은 이 표를 같이 고친다.
...
**`replacement.confidence`는 라벨러에게 보이지 않는다.** §4.4 `replacement`에는 있지만 `LABELER_REPLACEMENT_FIELDS`가 `code`·`match_method` 둘뿐이다. 보이지 않는 동안의 규칙은 §6.2.2 근거 ① 구간 상한 4번에 있다.

**`--with-extra-context`는 이제 효과가 없다.** `issue_bodies`·`pr_labels`가 기본 노출로 옮겨지면서(#113) `classify/sampling.py`의 이 옵션은 호환용으로만 남았다. 붙이든 안 붙이든 같은 파일이 나온다.
...
4. **`replacement.confidence`를 볼 수 없으면 0.8 미만으로 취급한다.** 지금 라벨링 파일에는 이 값이 없다 (§2.1 — `LABELER_REPLACEMENT_FIELDS`가 `code`·`match_method`뿐이다). 확인할 수 없는 확신에 기대지 않으므로, **화이트리스트에 `confidence`가 들어올 때까지는 3번이 모든 근거 ①에 적용된다** — 근거 ①만으로는 0.5~0.8이고, 0.8 이상은 ②~⑥의 뒷받침이 있을 때만이다. 이 제약은 문서 끝 선행 조건 표 7번이 해소되면 풀린다.

#### 6.2.3 `replacement`를 읽는 법 — `code`가 `null`인 건이 여전히 많다

**예비 200건 파일은 `replacement`가 200건 모두 `null`이었다** (게이트 1은 이 상태로 측정했다). 대체 코드 매칭이 그 뒤에 붙었다 (#67 → PR #103, #107 → PR #108). 500건 파일은 `replacement`가 채워진 채로 나온다. **다만 채워진다는 것이 `code`가 있다는 뜻은 아니다.** 같은 예비 200건을 새 파이프라인으로 다시 돌린 결과(PR #103·#108)가 이렇다:

| `match_method` | 건수 | `code` | 뜻 |
|---|---:|---|---|
| `SAME_LOCATION` | 39 | 39건 채워짐 (자식 파일 원문을 넘겼을 때. 안 넘기면 8건) | 삭제된 함수와 같은 이름의 함수가 같은 커밋·같은 파일에 추가됐다 |
| `NONE` | 81 | 항상 `null` | 그 커밋이 그 파일에 추가한 줄이 **하나도 없다** |
| `null` | 80 | 항상 `null` | **판정 불가.** 이름이 다른 함수가 들어왔거나, 부분 수정뿐이거나, 판정 재료가 없다 |
...
| 7 | **(가능하면) `replacement.confidence`를 라벨러 화이트리스트에 넣는다** | §6.2.2 근거 ① 구간 상한이 이 값에 기댄다. 지금은 `LABELER_REPLACEMENT_FIELDS`가 `code`·`match_method`뿐이라 라벨러가 볼 수 없고, 그래서 4번 규칙(볼 수 없으면 0.8 미만 취급)이 **모든 근거 ①을 0.5~0.8로 묶는다.** 파이프라인은 `SAME_LOCATION` 39건 중 36건에 0.9를 준다. 넣을 때는 §2.1 표를 함께 고친다 (§2.1 "함께 바뀌어야 한다") | 희수 (`classify/sampling.py`) — 이슈 미등록 |
