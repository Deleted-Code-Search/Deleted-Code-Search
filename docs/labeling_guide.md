...
| 2 | `tools/label_cli.py`가 UNKNOWN일 때 원인 태그를 강제 | 게이트 1에서 UNKNOWN 62건 중 43건이 무태그였다. 조건은 "4종 중 1개 이상 **또는** `filter-miss`" (§6.3.2) | #88 — **완료.** 새로 저장하는 라벨에만 강제하고, 예비 200건(v1) 파일은 읽기·집계에서 검사하지 않는다 |
