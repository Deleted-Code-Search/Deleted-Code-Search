...
**판단이 서지 않으면 달지 않는다.** 평소대로 라벨하고(UNK면 §6.3.2 원인 태그도 평소대로), `note`에 사유를 적는다. #80 판정 C인 `717c77ac`(`any_schema`, "관련 구현의 교체/재작성 성격이 강해 명확한 이동/리네임으로 판정하기 어려움")가 이런 경우다. **이때 `note`에 `filter-miss` 낱말을 쓰지 않는다.** `label_cli`와 집계는 `note`에서 태그를 낱말로 찾으므로 `filter-miss 의심`도 `filter-miss`로 센다. `이동 의심: pydantic/_internal/x.py의 y` 처럼 적는다.
...
| v2 (유지) | 2026-09-24 | **`filter-miss` 기준을 순수 이동·리네임으로 좁힘 (#132).** #80 전수 조사에서 예비 200건 `filter-miss` 18건 중 9건이 이동이 아니었고(대부분 같은 자리 재작성) 라벨러마다 쓰는 법이 달랐다. ① §6.3.3 신설 — 다는 경우(본문이 거의 그대로인 이동·리네임, 0.9에 조금 못 미치는 것, 가까운 다른 커밋으로의 이동 — 재등장 커밋을 `note`에)와 달지 않는 경우(같은 자리 재작성, 옮기며 재작성, 부모 커밋에 이미 있음, 실제 삭제), #80 재헌 판정을 인용한 경계 예시(A 2건 + 0.9 미만 재작성 3건 + B 3건 + C 1건), 판단이 안 서면 달지 않고 `note`에 `filter-miss` 낱말도 쓰지 않는다, 이 경계를 택한 이유, #89 정밀도 판정도 이 경계를 쓴다 ② §4 DESIGN·§6.3.1-3에서 §6.3.3을 가리킴. **태그 어휘·기록 방식(`UNK` + `UNKNOWN` + `filter-miss`)·`label_cli` 검증·ADR-014·이유 8종·라벨 스키마는 그대로다.** 판정 경계를 좁혔지만 v2로 저장된 라벨이 0건이라 `guide_version`은 v2를 유지한다 | 없음 (v2 라벨 0건, 예비 200건은 v1) | 성제 |
