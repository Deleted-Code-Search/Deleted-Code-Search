...
- 날짜: **2026-09-17**
- 상태: 채택
- 관련 이슈: #62 (이 ADR), #63 (구현, 재헌), #33 (예비 200건 샘플 추출, 희수)
...
## 결정
- PARTIAL 레코드는 **`deleted_body`가 5줄 이상일 때만** 데이터셋과 라벨링 대상에 포함한다.
...
## 대안과 버린 이유
- **PARTIAL 전부 포함** — pydantic에서 4줄 이하가 PARTIAL의 85.1%(16,749건)다. 라벨링 표본 대부분이 함수 수정으로 채워진다.
- **PARTIAL 전부 제외 (FULL_FUNCTION만)** — 5줄 이상 PARTIAL 2,941건을 버린다. FULL_FUNCTION 3,001건과 비슷한 규모다.

## 한계
- **5줄이라는 값은 게이트 1 라벨링에서 재검토할 수 있다.** 5-9줄 구간에서 UNK 비율이 높게 나오면 기준을 올린다.
...
- **200건 예비 라벨링 샘플 추출(#33, `classify/sampling.py`, 희수)이 이 기준을 적용해야 한다.** 별도 이슈는 아직 없다.
